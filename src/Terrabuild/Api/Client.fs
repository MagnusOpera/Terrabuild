...
module private Build =
    ...
    let startBuild headers branchOrTag commit configuration environment targets force retry ci: StartBuildOutput =
        { StartBuildInput.BranchOrTag = branchOrTag
          StartBuildInput.Commit = commit
          StartBuildInput.Configuration = configuration
          StartBuildInput.Environment = environment
...
type Client(space: string, token: string) =
    ...
    interface Contracts.IApiClient with
        member _.BuildStart branchOrTag commit configuration environment targets force retry ci =
            let resp = Build.startBuild headers branchOrTag commit configuration environment targets force retry ci
            resp.BuildId
