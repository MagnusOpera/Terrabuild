        ...
        |> List.map (fun operation ->
            ...
            | Some container, false ->
                ...
                let envs =
                    ...
                    |> String.join " "
                let args = $"run --rm --net=host --name {node.TargetHash} -v /var/run/docker.sock:/var/run/docker.sock -v {homeDir}:{containerHome} -v {tmpDir}:/tmp -v {wsDir}:/terrabuild -w /terrabuild/{projectDirectory} --entrypoint {operation.Command} {envs} -e DOTNET_SKIP_FIRST_TIME_EXPERIENCE=1 {container} {operation.Arguments}"
